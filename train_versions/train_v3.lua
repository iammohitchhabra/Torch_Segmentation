...
SegModel:add(cudnn.ReLU(true))
branch=nn.ConcatTable()

b1=nn.Sequential()
b1:add(cudnn.SpatialConvolution(6,5,1,5,1,2):init('weight', nninit.xavier, {dist = 'normal', gain = 1.1}))
b1:add(nn.SpatialBatchNormalization(5, 1e-3))
b1:add(cudnn.ReLU(true))
b1:add(nn.Transpose())
b1:add(cudnn.SpatialConvolution(5,5,5,1,2,1):init('weight', nninit.xavier, {dist = 'normal', gain = 1.1}))
b1:add(nn.SpatialBatchNormalization(5, 1e-3))
b1:add(cudnn.ReLU(true))
b1:add(nn.Transpose())
b1:add(cudnn.SpatialConvolution(5,5,3,3,1,1):init('weight', nninit.xavier, {dist = 'normal', gain = 1.1}))
b1:add(nn.SpatialBatchNormalization(5, 1e-3))
b1:add(cudnn.ReLU(true))

b2=nn.Sequential()
b2:add(cudnn.SpatialConvolution(6,5,1,5,1,2):init('weight', nninit.xavier, {dist = 'normal', gain = 1.1}))
b2:add(nn.SpatialBatchNormalization(5, 1e-3))
b2:add(cudnn.ReLU(true))
b2:add(cudnn.SpatialConvolution(5,5,5,1,2,1):init('weight', nninit.xavier, {dist = 'normal', gain = 1.1}))
b2:add(nn.SpatialBatchNormalization(5,1e-3))
b2:add(cudnn.ReLU(true))
b2:add(cudnn.SpatialConvolution(5,5,3,3,1,1):init('weight', nninit.xavier, {dist = 'normal', gain = 1.1}))
b2:add(nn.SpatialBatchNormalization(5, 1e-3))
b2:add(cudnn.ReLU(true))
...
SegModel:add(cudnn.SpatialFullConvolution(10,64,3,3,1,1):init('weight', nninit.xavier, {dist = 'normal', gain = 1.1}):noBias())
SegModel:add(nn.SpatialBatchNormalization(64, 1e-3))
SegModel:add(cudnn.ReLU(true))
SegModel:add(cudnn.SpatialFullConvolution(64,32,5,5,2,2):init('weight', nninit.xavier, {dist = 'normal', gain = 1.1}):noBias())
